...
export async function POST(req: Request) {
  try {
    ...
    const { data: credits } = await supabase.from("user_credits").select("balance, unlimited").eq("user_id", user_id).single();

    // Unlimited users never get deducted
    if (credits?.unlimited) {
      return NextResponse.json({ success: true, cost: 0, new_balance: 99999, unlimited: true });
    }
