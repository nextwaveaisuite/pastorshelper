...
export async function POST(req: Request) {
  try {
    ...
    const [{ count: totalUsers }, { count: totalSermons }, { count: totalSeries }, { data: recentUsers }, { data: recentSermons }, { data: pageViews }] = await Promise.all([
    ...
    (pageViews || []).forEach((v: { page: string }) => { viewsByPage[v.page] = (viewsByPage[v.page] || 0) + 1; });
    const last7: Record<string, number> = {};
    for (let i = 6; i >= 0; i--) {
      const d = new Date(); d.setDate(d.getDate() - i);
      last7[d.toLocaleDateString("en-AU", { weekday: "short", day: "numeric" })] = 0;
    }
    (pageViews || []).forEach((v: { created_at: string; page: string }) => {
      const key = new Date(v.created_at).toLocaleDateString("en-AU", { weekday: "short", day: "numeric" });
      if (key in last7) last7[key]++;
    });
    return NextResponse.json({ stats: { totalUsers: totalUsers || 0, totalSermons: totalSermons || 0, totalSeries: totalSeries || 0, viewsByPage, viewsByDay: last7, recentUsers: recentUsers || [], recentSermons: recentSermons || [] } });
  } catch (e) { console.error(e); return NextResponse.json({ stats: { totalUsers: 0, totalSermons: 0, totalSeries: 0, viewsByPage: {}, viewsByDay: {}, recentUsers: [], recentSermons: [] } }); }
}
